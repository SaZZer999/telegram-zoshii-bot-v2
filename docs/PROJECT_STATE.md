...
* Відомий баг (окрема майбутня задача, не пов'язана з Receipt V2.2): видалення запасів за природною фразою кількості не завжди працює. Приклад: команда «Видали молоко одна штука, воно вже не потрібно» при одночасній наявності в запасах `Молоко — 1 шт` і `Молоко — 14,5 л` дала відповідь «Не знайшов такого запису в запасах» замість вибору `Молоко — 1 шт` або запиту на уточнення.

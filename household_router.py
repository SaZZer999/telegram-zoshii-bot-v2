...
EXPLICIT_ADD_ITEM_PROMPT = (
    ...
    "Це список ЗАГАЛЬНИХ домашніх запасів/покупок, а НЕ лише їжі — побутова хімія, засоби гігієни, речі "
    "побуту тощо теж дійсні товари (напр. «автокрісло», «пральний порошок», «лампочка», «підгузки»). НІКОЛИ "
    "не відкидай товар і не клади його в unresolved_fragments лише тому, що він не їстівний.\n"
    ...
    "Інше їстівне.\n"
    "Категорія — це лише орієнтовне групування у списку, а НЕ перевірка, чи товар їстівний: обери найближчу "
    "за змістом категорію, а якщо жодна з категорій справді не підходить (напр. непродовольчий товар) — "
    "постав «Інше їстівне» як універсальну категорію за замовчуванням. Невизначеність із категорією — НЕ "
    "причина для unresolved_fragments: товар усе одно повертай у items з якоюсь категорією.\n"
    "Якщо частину тексту не можна безпечно розібрати на назву й кількість (незрозуміла кількість, вигадана "
    "сума, неоднозначна фраза) — опиши цей фрагмент у unresolved_fragments (масив рядків) і НЕ вигадуй товар "
    "для нього. Це стосується ЛИШЕ такого випадку — НІКОЛИ не використовуй unresolved_fragments через те, що "
    "товар не їстівний чи не підходить під жодну категорію. Завжди повертай це поле, навіть порожнім "
    "масивом.\n\n"
    "Відповідай ТІЛЬКИ валідним JSON, без Markdown і без тексту поза JSON:\n"
